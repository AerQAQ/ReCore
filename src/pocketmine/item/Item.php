...
class Item{
	...
	private static function initCreativeItems($readFromJson = false){
		...
		if(!$readFromJson){
			...
			// self::addCreativeItem(Item::get(Item::TRUNK2, 2)); // 已删除 162:2
            ...
            self::addCreativeItem(Item::get(Item::SPAWN_EGG, 20)); //Iron Golem
            self::addCreativeItem(Item::get(Item::SPAWN_EGG, 21)); //Snow Golem
            self::addCreativeItem(Item::get(Item::SPAWN_EGG, 44)); //Zombie Villager
            self::addCreativeItem(Item::get(Item::SPAWN_EGG, 93)); //Flash
            ...
            self::addCreativeItem(Item::get(Item::HOPPER));
	...
	public function __construct($id, $meta = 0, int $count = 1, string $name = "Unknown") {
		...
		// 修复：确保 meta 始终为整数，防止 null 值问题
		$this->meta = ($meta !== null && is_numeric($meta)) ? ((int)$meta) & 0xffff : 0;
