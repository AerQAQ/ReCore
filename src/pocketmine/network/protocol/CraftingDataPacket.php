...
class CraftingDataPacket extends DataPacket{
	...
	private static function writeEntry($entry, BinaryStream $stream, $isOldClient = false){
		...
		}elseif(!$isOldClient && $entry instanceof EnchantmentList){
			// 🥵 老客户端不发送附魔列表
	...
	public function encode(){
		...
		foreach($this->entries as $entry){
			if($this->isOldClient && $entry instanceof EnchantmentList){
				continue; // 老客户端跳过附魔列表
			}
			$filteredEntries[] = $entry;
		}
		
		$this->putInt(count($filteredEntries));

		$writer = new BinaryStream();
		foreach($filteredEntries as $d){
			$entryType = self::writeEntry($d, $writer, $this->isOldClient);
			if($entryType >= 0){
