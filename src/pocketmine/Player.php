...
use pocketmine\event\entity\MinecartInteractEvent;
...
if(!$this->isOldClient){
		...
		if(!$revert and ($delta > (1 / 16) or $deltaAngle > 10)){
			...
			if(!$isFirst){
				...
				if(!($revert = $ev->isCancelled())){ //Yes, this is intended
					...
					if($this->server->redstoneEnabled) $this->getLevel()->updateAround($ev->getTo()->round());//原注释
	...
	protected function processLogin(){
		if(!$this->server->isWhitelisted(strtolower($this->getName()))){
			$this->close($this->getLeaveMessage(), "本服务器已开启白名单！");
...
if(count($this->server->getOnlinePlayers()) >= $this->server->getMaxPlayers() and $this->kick("disconnectionScreen.serverFull", false)){
    break;
				...
				// 🥵 0.13客户端跳过版本检查
if(!$this->isOldClient && !in_array($packet->protocol1, ProtocolInfo::ACCEPTED_PROTOCOLS)){
						...
						if($this->startAction > -1 and $this->getDataFlag(self::DATA_FLAGS, self::DATA_FLAG_ACTION)){
							if($this->inventory->getItemInHand()->getId() === Item::BOW){
								...
								$nbt = new CompoundTag("", [
									...
									"Fire" => new ShortTag("Fire", ($this->isOnFire() or ($bow->getEnchantment(21) !== null)) ? 45 * 60 : 0)//Enchantment::TYPE_BOW_FLAME
								]);
								...
								if($ev->isCancelled()){
									...
									if($this->isSurvival()){ 
										if($bow->getEnchantment(22) === null){//TYPE_BOW_INFINITY
			...
			case ProtocolInfo::INTERACT_PACKET:
				...
				if($packet->action === InteractPacket::ACTION_RIGHT_CLICK){
					if($target instanceof Animal){
						...
						if($target instanceof Sheep and $item instanceof Shears){
							...
							$target->sethasWool(false);
							foreach($target->getDrops() as $item){
								$target->getLevel()->dropItem($target, $item);
							}
							break;
						}
						if($this->isSurvival()){
				...
				if($target instanceof Entity and $this->getGamemode() !== Player::VIEW and $this->isAlive() and $target->isAlive()){
					...
					foreach($item->getEnchantments() as $Enchantment){
						if($Enchantment->getId() == 9){ //TYPE_WEAPON_SHARPNESS
							$damageBase += ($Enchantment->getLevel() * 1.25);
						}elseif($Enchantment->getId() == 13){ //TYPE_WEAPON_FIRE_ASPECT
			...
			case ProtocolInfo::ENTITY_EVENT_PACKET:
				...
				switch($packet->event){
					case EntityEventPacket::USE_ITEM: //Eating
						...
						if($this->lastEat != PHP_INT_MAX and $EatTime <= 1.15 and isset($this->server->antiFastEat) and $this->server->antiFastEat){
    ...
    */
	...
	if($recipe instanceof ShapedRecipe){
		for($x = 0; $x < 3 and $canCraft; ++$x){
			for($y = 0; $y < 3; ++$y){
				$index = $y * 3 + $x;
				// 安全检查：确保数组键存在，且值为 Item 对象
				if(!isset($packet->input[$index]) || !($packet->input[$index] instanceof Item)){
					$canCraft = false;
					break 2;
				}
				$item = $packet->input[$index];
	...
	public function kick($reason = "", $isAdmin = true){
		...
		if(!$ev->isCancelled()){
			if($isAdmin){
				$message = "被踢出服务器" . ($reason !== "" ? " 原因: " . $reason : "");
			}else{
				if($reason === ""){
