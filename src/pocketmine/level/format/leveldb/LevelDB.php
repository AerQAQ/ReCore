...
class LevelDB extends BaseLevelProvider{
	...
	const NEW_VERSION = "\x2c"; //since 1.16.100? word","
	...
	public function __construct(Level $level, $path){
		...
		$ver = $this->readVersion(0, 0);
		if($ver === -1){
			throw new LevelException("Not Compatible decompressor(世界版本太新)");
		}
	}
	...
	public static function generate($path, $name, $seed, $generator, array $options = []){
		...
		$levelData = new CompoundTag("", [
			...
			//生存斧添加NBT
			"Platform" => new IntTag("Platform", 2), //总为2
			"Generator" => new IntTag("Generator", 1), //0:有限世界 1:无限世界 2:超平坦
			"StorageVersion" => new IntTag("StorageVersion", 4), //基岩版存储工具版本，0.14.1是4, 1.19为8。
			"NetworkVersion" => new IntTag("NetworkVersion", 45), //网络协议版本号,这里使用0.14.0的
			"currentTick" => new LongTag("currentTick", 0), //游戏刻
			"spawnMobs" => new ByteTag("spawnMobs", 0), //怪物生成
			"eduLevel" => new ByteTag("eduLevel", 0) //教育版世界
			
			//"LastPlayed" => new LongTag("LastPlayed", 0), //玩家在最后一次退出游戏时的Unix时间戳
			//"worldStartCount" => new LongTag("worldStartCount", 0), //最后一次打开存档时的时间
		]);
		$nbt = new NBT(NBT::LITTLE_ENDIAN);
	...
	//Some API5.0.0 function
	
	private function readVersion(int $chunkX, int $chunkZ){
		$index = self::chunkIndex($chunkX, $chunkZ);
		try{
			$chunkVersionRaw = $this->db->get($index . self::NEW_VERSION);
		}catch(\Throwable $e){
			if($e->getMessage() == "Not implemented: encountered a block compressed with an unknown decompressor"){
				return -1;
			}
		}
		if($chunkVersionRaw === false){
			$chunkVersionRaw = $this->db->get($index . self::ENTRY_VERSION);
			if($chunkVersionRaw === false){
				return null;
			}
		}
