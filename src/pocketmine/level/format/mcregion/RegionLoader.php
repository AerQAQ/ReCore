...
class RegionLoader{
    ...
    public function __construct(LevelProvider $level, $regionX, $regionZ){
        ...
        if($this->filePointer === false){
            throw new \RuntimeException("Could not open region file: " . $this->filePath);
        }
    ...
    public function readChunk($x, $z){
        ...
        $chunk = $this->unserializeChunk($chunkData);
    ...
    protected function loadLocationTable(){
        ...
        for($i = 0; $i < 1024; ++$i){
            $index = isset($data[$i + 1]) ? $data[$i + 1] : 0;
            $timestamp = isset($data[1024 + $i + 1]) ? $data[1024 + $i + 1] : 0;
            $this->locationTable[$i] = [$index >> 8, $index & 0xff, $timestamp];
    ...
    private function writeLocationTable(){
        ...
        for($i = 0; $i < 1024; ++$i){
            $write[] = isset($this->locationTable[$i][2]) ? $this->locationTable[$i][2] : 0;
        }
        fseek($this->filePointer, 0);
    ...
    protected function writeLocationIndex($index){
        ...
        fwrite($this->filePointer, Binary::writeInt(isset($this->locationTable[$index][2]) ? $this->locationTable[$index][2] : 0), 4);
    }
