 ...
 *
 * @author esprit
 * @link https://github.com/ReCoreTeam/ReCore/
 *
*/
...
class Item extends Entity{
	...
	protected function initEntity(){
		...
		// 读取配置，是否显示掉落物浮空文字
		$showNameTag = $this->server->getAdvancedProperty("item.drop.show-name-tag", true);

		if ($showNameTag) {
			// 设置浮空文字：显示 "物品名称 X 数量"
			$name = $this->item->hasCustomName() ? $this->item->getCustomName() : $this->item->getName();
			$count = $this->item->getCount();
			$displayName = $name . " x " . $count;
	...
	public function onUpdate($currentTick){
		...
		if($this->isAlive()){
			...
			// ====== 修复：直接使用配置值，不从server对象读取 ======
			// 从服务器配置中读取掉落物清理设置（使用默认值）
			$despawnEnabled = true;  // 默认启用
			$despawnTime = 6000;     // 默认5分钟 (6000 ticks)

			// 尝试从配置读取
			if(isset($this->server->properties["item-despawn-enabled"])){
				$despawnEnabled = (bool) $this->server->properties["item-despawn-enabled"];
			}
			if(isset($this->server->properties["item-despawn-time"])){
				$despawnTime = (int) $this->server->properties["item-despawn-time"];
			}
			// ====== 修复结束 ======

			// 根据配置决定是否清理掉落物
			if($despawnEnabled && $this->age > $despawnTime){
