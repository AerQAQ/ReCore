...
abstract class Entity extends Location implements Metadatable{
	...
	const DATA_AGEABLE_FLAGS = 14; const DATA_IS_BABY = 14;
	const DATA_NO_AI = 15;
	...
	const DATA_CREPPER_SWELL_DIRECTION = 16;
	
	const DATA_SHOOTER_ID = 17;
	const DATA_CREPPER_SWELL = 17;
	const DATA_CREPPER_SWELL_2 = 18;
	
	const DATA_RABBIT_TYPE = 18; const DATA_CAT_TYPE = 18;
	const DATA_JUMP_TYPE = 19;
	const DATA_BLOCK_INFO = 20; const DATA_WOOD_ID = 20;
	const DATA_IN_LOVE = 21;
	const DATA_ONFIRE = 32;
	const DATA_ZOMBIE_IS_BABY= 44;
	const DATA_VILLAGER_IS_BABY= 46;
	const DATA_ENDERMAN_HELD_ITEM_ID = 48;
	const DATA_ENDERMAN_HELD_ITEM_DAMAGE = 49;
	const DATA_ENDERMAN_TREMBLE = 50;
	...
	public function handleLavaMovement(){
		$block = $this->getLevel()->getBlock($this->temporalVector->setComponents(Math::floorFloat($this->x), Math::floorFloat($this->y), Math::floorFloat($this->z)));
	...
	public function moveFlying($dx, $dy, $dz){
		$this->motionX += $dx;
		$this->motionY += $dy;
		$this->motionZ += $dz;
	}
	...
	protected function recalculateBoundingBox(){
		...
		$this->boundingBox->setBounds(
			$this->x - $halfWidth,
			$this->y - $this->heightOffset,
			$this->z - $halfWidth,
			$this->x + $halfWidth,
			$this->y - $this->heightOffset + $this->height,
			$this->z + $halfWidth
		);
	}
