...
class ThrownPotion extends Projectile{
	...
	public function kill(){
		...
		// 喷溅水瓶浇灭火的逻辑
		if($this->getPotionId() === Potion::WATER_BOTTLE){
	...
	/**
	 * 浇灭水平相邻1格的火（包含自身，不包含对角线）
	 */
	private function extinguishFires() : void{
		$level = $this->getLevel();
		$pos = $this->getPosition()->floor();
		
		// 检查的位置：自身和水平四个方向（东、南、西、北）
		$positionsToCheck = [
			$pos, // 自身
			$pos->getSide(Vector3::SIDE_EAST),  // 东
			$pos->getSide(Vector3::SIDE_SOUTH), // 南
			$pos->getSide(Vector3::SIDE_WEST),  // 西
			$pos->getSide(Vector3::SIDE_NORTH)  // 北
		];
		
		foreach($positionsToCheck as $checkPos){
			$block = $level->getBlock($checkPos);
			if($block->getId() === Block::FIRE){
				$level->setBlock($checkPos, new Air(), true);
			}
		}
	}
